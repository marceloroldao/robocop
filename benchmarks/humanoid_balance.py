...
from robocop.controllers import PDController, FieldModulatedController, normalize
...
def run_episode(agent, seed: int, max_steps: int):
    ...
    return EpisodeResult(
        ...
        hit_rate=agent.hits / max(agent.lookups, 1),
    )
...
def print_summary(results):
    ...
    for name in names:
        ...
        print(f"Hit: {avg('hit_rate')*100:.2f}%")
...
def main():
    ...
    memory = DescriptiveMemory()
    print("FASE A - treino da memoria")
    for seed in range(900, 900 + args.train_seeds):
        r = run_episode(MemoryV5Agent(memory, True), seed, args.max_steps)
        print(f"seed={seed} passos={r.steps} hit={r.hit_rate:.2f} sims={r.sims_per_step:.2f}")
    memory.freeze()
    print("Memoria:", memory.stats())

    results = []
    for seed in range(1000, 1000 + args.seeds):
        for agent in (BaseAgent(), FullFieldAgent(), ConfidenceV3Agent(), MemoryV5Agent(memory, False)):
            r = run_episode(agent, seed, args.max_steps)
            results.append(r)
            print(f"{r.controller:16s} seed={seed} passos={r.steps} E={r.energy:.6f} CPU={r.cpu_ms:.3f} sims={r.sims_per_step:.2f} hit={r.hit_rate:.2f}")

...
class BalanceReflexMemory:
    """Memory of corrective reflexes indexed by posture and motion tendency."""
    ...
    @classmethod
    def fit(cls, recorders: Iterable[TransitionRecorder], prefall_window: int = 12,
            min_improvement: float = 0.003, min_after_stability: float = 0.62) -> "BalanceReflexMemory":
        features = []
        candidates = []
        ...
        prototypes = [BalanceReflexPrototype(np.asarray(feature, dtype=float),
                    np.asarray(sample.action, dtype=float).copy(), float(gain), float(after_s),
                    float(sample.energy), float(sample.reward))
                    for feature, sample, gain, after_s in candidates]
        return cls(prototypes, mean, std)

    def lookup(self, previous: SensorSnapshot, current: SensorSnapshot, k: int = 12) -> Tuple[Optional[np.ndarray], float, float]:
        ...
        best_idx = max(idxs, key=lambda i: (

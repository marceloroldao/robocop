...
def test_opposite_motion_retrieves_opposite_reflex():
    ...
    p_prev = plus.samples[0].after
    p_cur = plus.samples[1].before
    m_prev = minus.samples[0].after
    m_cur = minus.samples[1].before

    a_plus, gain_plus, _ = memory.lookup(p_prev, p_cur)
    a_minus, gain_minus, _ = memory.lookup(m_prev, m_cur)
...
def test_reflex_memory_excludes_terminal_samples():
    ...
    bad = TransitionSample(
        r.samples[-1].before,
        np.full(17, 0.3),
        snap(0.90, 0.3, 2.0, -1.0),
        0.0,
        0.09,
        True,
        2,
